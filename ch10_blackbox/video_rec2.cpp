...
double getStorageUsage()
{
    ...
    printf("getStorageUsage: blocks = %ld, avail = %ld, Usage = %f\n", MP.size.blocks,MP.size.avail, (double)(MP.size.blocks - MP.size.avail)/MP.size.blocks);
...
void *UpdateFrame(void *arg)
{
    ...
    while(1){
        ...
        printf("UpdateFrame: Working File Full Path = %s\n",buffer);

        puts("Before: output vidoe open");
        outputVideo.open(buffer, VideoWriter::fourcc('X', 'V', 'I', 'D'), 30, size, true);
        puts("After: output vidoe open");
        ...
        while(1)
        {
            ...
            puts("IMSHOW2");              
            //화면에 영상을 보여줌
            imshow("Outputs", tempFrame);
            puts("outputVideo << tempFrame");
            outputVideo << tempFrame;
            // pthread_mutex_unlock(&frameLocker);
            
            printf("UpdateFrame: Sec %d ", atoi(getTimeStringFormat(persec)));
